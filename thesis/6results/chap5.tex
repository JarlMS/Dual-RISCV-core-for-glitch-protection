...
% \autoref{fig:power_comapre} shows the difference in power consumption through leakage, internal, switching and in total for both cores. 
...
\begin{table}[h!]
\centering
\caption{Area reports for CV32E40S and CV32E40DC.}
...
\midrule
\rowcolor{black!20} CV32E40S & 13525 & 42548.220 & 20572.873 & 63121.093\\
CV32E40DC & 29351[$+117.01\%$] & 97142.022[$+128.31\%$] & 43445.210[$+111.18\%$] & 140587.232[$+122.73\%$] \\ 
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[h!]
\centering
\caption{Clock cycles used to execute the code in \autoref{lst:sample_code}, \autoref{lst:test_code} and \autoref{lst:coverage_test} on both cores.}
\label{tab:test_ccs}
\begin{tabular}{c|ccc}
\toprule 
...
The glitch attack was successful, and line \textit{21} in \autoref{lst:test_code} was executed. The waveforms from simulation are shown in \autoref{fig:instr_skip_single_wave}. From the figure we see that the glitch is detected immediately and a major alert is raised. The test is \textbf{passed}.
...
The glitch attack was not successful. The attempted instruction skip is detected and a major alert is raised. This can be seen in the simulation waveforms in \autoref{fig:instr_skip_loop_single_wave}. The test is \textbf{passed}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{docs/images/instr_skip_glitch_injection_single_core.png}
    \caption{Waveforms from simulated skip of \textit{c.j} instruction to jump out of loop on CV32E40S.}
...
The glitch attack was not successful. However, no major alert was raised by the core as can be seen in \autoref{fig:direct_skip_single_wave}. This glitch therefore bypassed the PCH feature and the test is \textbf{not passed}.
...
\begin{figure}[h!]
    ...
    \caption{Waveforms from simulated glitched LW instruction skip on CV32E40DC.}
...
\begin{figure}[h!]
    ...
    \caption{Waveforms from simulated glitched SW instruction skip on CV32E40DC.}
...
Adding an extra core is also shown in \autoref{subsec:power} to increase total area and total power consumption. Because the CV32E40DC consists of two cores, synchronization registers and comparators we would expect to see at least a $100\%$ increase in both metrics. \autoref{fig:area_power_plot} shows the difference between these metrics for both cores. The red bars represent the measured values from the CV32E40DC and the blue represent those from the CV32E40S. The axis on the left shows the area in $pm^2$ and the axis on the right shows the power consumption in $W$. From the figure we see that the total area increases by $122.73\%$ and the total power increases by $189.34\%$. 

This increase in area and power consumption is more than doubled. While PPA can be traded for robustness in secure operation, we still want to save on energy and cost whenever possible when manufacturing a chip. However, with optimizations like for example using a shared register file for the cores within the CV32E40DC, it is possible to bring the PPA metrics closer to those of the CV32E40S. This will make it a more viable alternative to the CV32E40S. 

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{docs/images/metric_compare.png}
    \caption{Area and power consumption comparison after synthesis of CV32E40S and CV32E40DC. }
    \label{fig:area_power_plot}
\end{figure}

\autoref{tab:detection} shows whether the glitch attack was detected or not for all tests. Detecting instruction skips is shown to be more reliable using the CV32E40DC. This core was able to detect a glitch in all three test cases. The CV32E40S was only able to detect a glitch in two of the three tests, as the glitch skipping directly to the end and the coverage tests went undetected. For unknown reasons, the skipping of a function call was only successful on the CV32E40S as described in \autoref{subsubsec:func_call}. Repeating the test lead to the same result every time and random chance can therefore be ruled out. The reason why this test has a different outcome on both cores could be due to some undiscovered difference in timing between them.


\begin{table}[h]
\centering
\caption{Glitch detection in the tests from \autoref{tab:instruction_fault_tests} on the CV32E40S and CV32E40DC.}
\label{tab:detection}
